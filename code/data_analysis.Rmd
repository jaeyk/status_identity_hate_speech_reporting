...
reply_vec <- names(df)[str_detect(names(df), "reply_attitude|reply_socialnorm|reply_liking|reply_user_attitude|reply_user_realname|reply_report|reply_upvotes")]
...
```{r}
comment_plot <- custom_group_plot(comment_summ, "Comment", c("Attitude on Comment = Hate Speech", "Willingness to Report", "Attitude on Commenter", "Willingness to Like", "Atttidue on Comment"))

ggsave(here("outputs", "comment_summ.png"), width = 11)

platform_plot <- custom_group_plot(youtube_summ, "Platform", c("Attitude on Gov Regulation", "Attitude on Self-regulation", "Attitude on YouTube"))

ggsave(here("outputs", "youtube_summ.png"), width = 11)

reply_plot <- custom_group_plot(reply_summ, "Reply", c("Many upvotes", "Willingness to Like", "Peer pressure", "Real name", "Willingness to Report", "Attitude on Replier", "Attitude on Reply")) +
    labs(subtitle = "Difference in means between treatment groups (focusing on upvote effect)")

ggsave(here("outputs", "reply_summ.png"), width = 11)

law_plot <- custom_group_plot(law_summ, "Legislation", c("Support for Gender Discrimination Law", "Support for Hate Speech Law"))

ggsave(here("outputs", "law_summ.png"), width = 11)

metoo_plot <- custom_group_plot(metoo_summ, "Social Movement", c("Attitude on MeTooMovement"))
...
```{r}
(comment_plot / reply_plot / platform_plot / law_plot / metoo_plot) + plot_annotation(
    title = "Difference in means estimation",
    tag_levels = 'A')
    
ggsave(here("outputs", "all_summ.png"), height = 12, width = 12)
```
